...
init_instance() {
    ...
    new_json="$(jq '.resource_limits.user_space_size = "40000MB" |
        ...
        .process.default_stack_size = "512MB" |
        ...
        .env.default = [ "LD_LIBRARY_PATH=/usr/lib/jvm/java-11-alibaba-dragonwell/jre/lib/server:/usr/lib/jvm/java-11-alibaba-dragonwell/jre/lib:/usr/lib/jvm/java-11-alibaba-dragonwell/jre/../lib","SPARK_USER=root","SPARK_HOME=/bin","SPARK_SCALA_VERSION=2.12","SPARK_CONF_DIR=/bin/conf" ]' Occlum.json)" && \
...
build_spark() {
    ...
    cp -rf ../spark-3.1.1-bin-hadoop2.7/* image/bin/
    cp /usr/bin/setsid image/bin
    cp ../bash-static/bin/bash image/bin/
